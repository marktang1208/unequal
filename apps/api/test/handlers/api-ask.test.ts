...
describe("api-ask handler (CP-7-D #1/#2)", () => {
  ...
  // P-1MB fix: 1000 chunks 模拟大数据场景 → handler 必须传 limit=8 给 DB（防 CloudBase 1MB 阻塞；chunk avg 87KB）
  it("1000 chunks mock 不 throw：handler 传 limit=8 给 DB + 仍能 topK=5", async () => {
