/**
 * api-ingest handler（CP-6 Phase 2 stub → Phase 4 完整实现）
 * POST /api-ingest
 */
import {
  errorResponse,
...
export async function main(event: HttpTriggerEvent): Promise<HttpTriggerResponse> {
  ...
  // TODO Phase 4: chunking + embedding + DB writes (admin auth + ADMIN_TOKEN header)
  return errorResponse("NOT_IMPLEMENTED", "api-ingest stub (CP-6 Phase 2)", 501);
}