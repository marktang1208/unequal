/**
 * api-sessions-list handler（CP-6 Phase 2 stub → Phase 3 完整实现）
...
export async function main(event: HttpTriggerEvent): Promise<HttpTriggerResponse> {
  ...
  // TODO Phase 3: JWT auth + chatSession.where({userId}).orderBy().get()
  return errorResponse("NOT_IMPLEMENTED", "api-sessions-list stub (CP-6 Phase 2)", 501);
}