...
**完成日期**：2026-06-17
**State 详情**：`docs/superpowers/state-cp6.md`
...
| D1 SQL → CloudBase NoSQL（9 collection）| ✅ code 完成；需 CloudBase 控制台建 collection + index |
| CloudBase SDK init + env 硬验证 | ✅（spec §7.3）|
| 4 secrets 注入 | ⏸ 待 CloudBase 环境创建 |
| 8 vars 注入 | ⏸ 待 CloudBase 环境创建 |
| CloudBase 环境创建 + 9 collection + 13 函数部署 | ⏸ 待用户手动操作 |
| 6 步 smoke 端到端 | ⏸ 待 CloudBase 部署后 |
| v0 CF 部署封存 | ⏸ 本 commit 同步 tag `v0-cf-archived` |
...
| apps/miniprogram | 32 |
| apps/crawler | 19 |
| **累计** | **143**（CP-5 287 → CP-6 -144 重写后净 -144）|

### v0 封存

v0 CF 真接部署（commit `f623f66`）已封存归档：

- **git tag**：`v0-cf-archived`（CP-6 收尾 commit 同时打）
...
详见 `docs/superpowers/state-cp5.md`（CP-5 收尾 + 封存附录）

### 下一步（cp-7 候选）

1. **CloudBase 环境创建 + 资源部署**（用户操作）
2. **Mini-program 真机验证**（需 AppID 注册时同时做）
3. **KEK 轮换演练**（M6.8 留口）
4. **v0 资源销毁决策**（1 个月后）
