...
- **v2.3 架构修正**（admin 不 embed + API 自己 embed）：`docs/archive/state/state-arch-v2.3.md`
- **P3-7 spec**：`docs/superpowers/specs/2026-06-22-cp7-p3-7-crawler-manual-push-design.md`
- **P3-7 种子库 spec**：`docs/superpowers/specs/2026-06-22-p3-7-seeds-design.md`
- **P3-7 plan**：`.claude/plans/cp7-p3-7-crawler-manual-push.plan.md`
- **P3-7 种子库 plan**：`.claude/plans/p3-7-seeds.plan.md`
- **测试文章**：https://mp.weixin.qq.com/s/50y5re6jivLGLzd5fTtaTA

---

## 14. P3-7 v1 简化补完：种子 URL 库（2026-06-22）

P3-7 真接 4 暴露：`trigger.ts resolveSeedUrls` v1 简化返空，fullScan / limit 模式跑不出数据。spec 决策"minipgm 不增任何上传入口"（见顶层 spec §14 spec drift），P3-7 走"admin MacBook 本地爬虫 + 本地暂存 + 手动批量推"工作流已闭环，**差种子 URL 库**就 run 起来。

### 14.1 Spec + Plan

- **Spec**：`docs/superpowers/specs/2026-06-22-p3-7-seeds-design.md`（commit `6523415`，10 节 315 行）
  - JSON 文件 source of truth（`apps/crawler/seeds/{source}.json`，git 跟踪）
  - SQLite `crawler_seeds` 表 UI 视图（runtime 状态：last_crawled_at / last_status / retry_count）
  - admin UI `/seeds` 4 tab + 增删改 + 批量粘贴
  - 3 retry + failed 标记
- **Plan**：`.claude/plans/p3-7-seeds.plan.md`（3 phase / Medium）

### 14.2 2 commit 实施链

| Commit | 内容 |
|---|---|
| `1f30a54` | Phase A+B：抽 json-io + SeedsStore + SeedsLoader（packages/local-llm 共享） |
| `a9b2047` | Phase B+真接：SeedsMiddleware + SeedsPage UI + 3 真接场景全 PASS |

### 14.3 关键设计决策

- **数据存储**：JSON 文件 source of truth + SQLite UI 视图
- **同步策略**：UI 改即写 JSON（单 admin 无冲突；多设备 → v2 考虑）
- **去重策略**：`last_crawled_at ASC`（未拉过优先）
- **trust_level**：URL 上指定（覆盖默认 xhs=0 / wechat-mp=2 / webpage=1）
- **失败处理**：3 retry（退避 1/3/10s）→ 失败写 `last_status=failed`
- **health check**：不做（爬取反馈已足够）
- **UI 位置**：独立新页 `/seeds`（不嵌弹窗）

### 14.4 真接 3 场景全 PASS

| # | 场景 | 结果 |
|---|---|---|
| 1 | admin dev → POST /api/seeds → JSON 同步到 `apps/crawler/seeds/webpage.json` | ✅ |
| 2 | `pnpm -F crawler start --source=webpage --limit=2` → seeds-loader 读 SQLite → 拉 2 条 URL | ✅（fetch 404 但链路通） |
| 3 | failed URL 写 `last_status=failed` + `last_error="fetch ... failed: HTTP 404"` + `retry_count=1` | ✅ |

### 14.5 SeedsStore 计划外微调

T2 时 SeedsStore 放 admin 端（plan 建议 admin-only），但 crawler test 跨包 import admin 触发 rootDir 冲突。T5 中途迁到 `packages/local-llm/`（与 StatusStore 同位置，monorepo 共享），crawler 端可正常 import + 跨包不破坏 rootDir。**已 commit** `1f30a54` 包含此迁。

### 14.6 累计测试

- 全 monorepo **449 tests** all PASS（minipgm 49 + local-llm 51 + shared 58 + crawler 49 + api 129 + admin 162）
- typecheck：6 workspaces 干净
