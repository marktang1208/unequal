...
| 资产 | 说明 | 处理 |
|---|---|---|
| `apps/api/src/handlers/api-debug-chunks.ts` | dump 该 user chunks（含 embeddingLen 验证）| 留作 CP-7-C 范围 |
| `apps/api/src/handlers/api-debug-docs.ts` | dump 该 user docs | 留作 CP-7-C 范围 |
...
CP-7-C 候选（独立项目）：
1. 删除临时 debug handlers 或加 IP allowlist 强保护
2. ingest user_id 参数加更严格鉴权（admin scope + audit log）
3. crawler CLI 集成 user_id 支持
4. 修 `db.add()` 让 schema `id` 字段自动填（避免污染）
5. 修 `tcb fn deploy` 流程内化 env vars push（state-cp7-b §6.3 教训 #7）
6. 把 documents 的 schema id 字段补回（数据迁移）
