...
12. **queryString 字段兼容修** — P3.7 完成，详见 §10.4
13. **ask handler citations title="未知文档"** — P3.8 留：chunk.documentId 在 docMap 里没找到匹配，docMap 用 `_id` key 但 chunk.documentId 是 schema 字段（非 _id）。修法：upload 时把 doc._id 也存到 chunk.documentId（或额外加 documentIdByDocId 反向索引）。cosmetic 不阻塞，留 P4。
...
## 10.5 P3.8 (cosmetic, 留 P4) ask citations title="未知文档"

ask handler 3 处 citations 的 `title: "未知文档"` — chunk.documentId 没在 docMap 找到匹配。

**根因**：`docMap` 用 `d._id` 作 key（cloudbaserc auto-generated doc id），但 chunk.documentId 是 schema 字段（upload 写时给的是新 `newId()`，不是 doc._id）。两者不一致。

**修法（不阻塞，留 P4）**：
- upload 时把 `document._id` 也写到 `chunk.documentId`（一致性）
- 或者 docMap 用 `d.id` 作 key（upload 写 chunk 时用同一 newId）

cosmetic 不阻塞 smoke 6/6 全通，但 user 体验差（看不到文档名）。
