...
### 8.2 部署自动化（CP-6+ 新增；P3.5 重写 collections/indexes）
...
| `deploy-secrets.ts` | `pnpm -F api deploy:secrets` | 注入 4 secrets + 8 vars（HTTP API — **待重写为 tcb CLI**）| `TCB_ACCESS_TOKEN` |
...
pnpm deploy:secrets        # 注入 4 secrets + 8 vars（待 P3.6 重写为 tcb fn config）
```
...
8. KEK 轮换演练 (M6.8)
9. v0 资源销毁决策 (1 个月后)
10. ~~**cloudbaserc.smoke.json secrets cleanup** — smoke 后跑一次 `tcb --config-file cloudbaserc.json fn deploy` (7 vars 干净版) 清 4 secrets~~ ✅ 已流程化（每次 smoke 完跑干净 deploy 清 12 → 7 vars）
