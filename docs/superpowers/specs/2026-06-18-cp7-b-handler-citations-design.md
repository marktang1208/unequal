...
| R-5 | minipgm `deleteSession` 用 path param `/api-sessions-delete/${id}`，但 server handler `getQuery(event, "id")` 期望 query → 真接时 400 INVALID_REQUEST（CP-7-A 遗留路径风格不一致）| caller 改 query style：`path: "/api-sessions-delete"` + `query: { id }`，对齐 sessions-delete handler 实际契约 |
...
deleteSession(id): Promise<void>:
  改 cloudCall({ path: "/api-sessions-delete", httpMethod: "DELETE", query: { id }, jwt })
  （修复 CP-7-A 遗留：原 path param 风格与 handler query 不一致 → 真接 400）
...
- [ ] AC-7a `deleteSession` caller 改 path + query 风格（修复 CP-7-A 遗留）
...
| 风险 | 缓解 |
|---|---|
| [N] 解析越界（LLM 返 [9] 但 top 只 5）| 越界丢弃，不报错；AC 显式覆盖 |
| [N] 重复（[1][1]）| 去重保 first；AC 覆盖 |
| LLM 不引任何 [N] → 空 citations vs 旧 top-5 | 显式接受空（用户能看到"无引用"）；旧行为 100% 返 top-5 误导性更强 |
| message-bubble 渲染性能（长答案 → 多 segments）| 答案通常 < 500 字，segments < 20 项；远低于 wxml wx:for 性能阈值 |
| deleteSession 改 path 风格影响现有测试 | api.test.ts 改 1 个 mock 断言；AC-7a 显式列 |
| updateNickname 404 边界（user record 不存在）| wx-login 后必存在；理论 0 触发；AC 覆盖测试但路径 404 而非 upsert |
